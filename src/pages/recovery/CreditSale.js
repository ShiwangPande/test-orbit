...
function CreditSale({ petrodata }) {
    ...
    const handleEditChange = (e) => {
        ...
        setEditData(prevData => {
            ...
            if (name === 'selectedFuel') {
                const selectedNoozleData = itemPriceList.find(item => item.Item.name.toLowerCase() === value.toLowerCase());
                ...
                if (selectedNoozleData || selectedItemData) {
                    updatedData.rate = selectedNoozleData.price;
            ...
            if (selectedFuel === 'HSD' || selectedFuel === 'MS' || selectedFuel === 'POWER') {
    ...
    const handleSubmit = async (e) => {
        ...
        const gstype = itemPriceList.some(item => item.Item.name === selectedFuel);
        if (!validateForm()) {
        ...
        try {
            ...
            const [msHsdResponse, otherResponse] = await Promise.all([
    ...
    const handleSelectCustomer = (customer) => {
        ...
        const defaultRate = selecteditemPriceList ? selecteditemPriceList.Item.price : (selectedItem ? selectedItem.Item.mrp : '');
        setRate(defaultRate);
    ...
    // useEffect(() => {
    //     if (Array.isArray(itemPriceList) && itemPriceList.length > 0) {
    //         const firstItem = itemPriceList[0].Item; // Access the first element's Item property
    //         setSelectedFuel(firstItem.name); // Check if the name is 'HSD'
    //         setRate(firstItem.price);
    //         setSelectedFuelId(firstItem.id);
    //         setEditData(prevData => ({
    //             ...prevData,
    //             selectedFuel: 'HSD',
    //             selectedFuelId: firstItem.id,
    //             rate: firstItem.price
    //         }));
    //     }
    // }, [itemPriceList]);
    ...
    const handleSelectfuel = (selectedValue) => {
        ...
        const allItems = [...itemPriceList, ...ItemName];
        const selectedItem = allItems.find(item => item.Item.name === selectedValue);

        if (selectedItem) {
            const { id, price, mrp } = selectedItem.Item;
            ...
            calculateTotalAmt(quantity, price || mrp, mrp);
        }
    ...
    useEffect(() => {
        ...
        const isSelectedFuelFromPriceList = itemPriceList.some(item => item.Item.name === selectedFuel);

        if (isSelectedFuelFromPriceList) {
            ...
            setSgst(calculatedSgst.toFixed(2));
            setgst(gst_petrol_percentage);
            setInclusiveTotal((total - calculatedCgst - calculatedSgst).toFixed(2));
        }
    }, [totalAmt, ItemName, selectedFuel, petrolgst]);
    ...
    const calculateTotalAmt = (quantity, rate) => {
        ...
        if (!isNaN(quantity) && !isNaN(effectiveRate)) {
            ...
            const total = parseFloat(effectiveQuantity) * parseFloat(effectiveRate);
    ...
    const handleTotalAmtChange = (e) => {
        ...
        const effectiveRate = parseFloat(rate) || parseFloat(editData.mrp);
                                                ...
                                                <div className="flex flex-col col-span-1 gap-1">
                                                    ...
                                                    <div className="mt-1 relative">
                                                        ...
                                                        >
                                                            ...
                                                            {[...itemPriceList, ...ItemName].map((item) => (
                                                ...
                                                <div className="flex flex-col col-span-1 gap-1">
                                                    ...
                                                    <div className="mt-1 relative">
                                                        <input
                                                            type="number"
                                                            value={rate}
                                                            readOnly={isSelectedFuelFromPriceList}
                                                            disabled={isSelectedFuelFromPriceList}
                                                            onChange={(e) => {
                                                                const value = e.target.value;
                                                                if (!isNaN(value) && value.length <= 7) {
                                                                    const numericValue = parseFloat(value);
                                                                    setRate(numericValue);
                                                                    calculateTotalAmt(quantity, numericValue, editData.mrp);
                                                                }
                                                            }}
                                                            placeholder="Rate"
                                    ...
                                    >
                                        ...
                                        {voucher.SalesDetail && voucher.SalesDetail.length > 0 && voucher.SalesDetail.map((detail, detailIndex) => (
                                            <div key={detailIndex} className="lg:my-2 my-1 grid grid-cols-2 lg:grid-cols-2 lg:gap-2 gap-1 lg:text-base text-xs">
                                                ...
                                                <p className="text-gray-700 col-span-2 font-semibold">
