...
function Receipt({ petrodata }) {
    ...
    useEffect(() => {
        const storedData = JSON.parse(
            localStorage.getItem("submittedReceiptData")
        );
        if (storedData) {
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            ...
            const existingData =
                JSON.parse(localStorage.getItem("submittedReceiptData")) || [];
    ...
    const handleRemove = (index) => {
        ...
        localStorage.setItem("submittedReceiptData", JSON.stringify(updatedData));
        setSubmittedData(updatedData);
    };
