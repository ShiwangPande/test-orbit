...
function Expenses({ petrodata }) {
    ...
    const validateEditForm = () => {
        ...
        if (!editData.selectedLedgerName || editData.selectedLedgerName.trim() === '') {
    ...
    useEffect(() => {
        const storedData = JSON.parse(localStorage.getItem('submittedExpensesData'));
        if (storedData) {
    ...
    const handleSubmit = (e) => {
        ...
        if (validateForm()) {
            ...
            const existingData = JSON.parse(localStorage.getItem('submittedExpensesData')) || [];
            ...
            localStorage.setItem('submittedExpensesData', JSON.stringify(existingData));
    ...
    // Function to handle selection of LedgerName
    const handleSelectLedgerName = (LedgerName) => {
        setSelectedLedgerName(LedgerName.Ledger.name);
        // setLedgerId(LedgerName.Ledger.id);
        setSearchQuery(LedgerName.Ledger.name);
        setShowDropdown(false); // Hide the dropdown after selecting
        setEditData((prevState) => ({
            ...prevState,
            selectedLedgerName: LedgerName.Ledger.name
        }));
    ...
    const handleAmountChange = (e) => {
        ...
        if (amountValue > 10000000) {
            ...
            setErrors((prev) => ({ ...prev, amount: 'Driver Cash results in quantity exceeding 10,000,000' }));
        } else {
    ...
    return (
        ...
        <div className="h-full min-h-screen flex overflow-hidden  bg-gradient-to-t from-gray-200 via-gray-400 to-gray-600 ">
            ...
            <main className="flex-1 relative z-0 overflow-y-auto focus:outline-none">
                <h1 className='relative block  lg:hidden text-white mx-auto w-[70%] text-center top-4 text-2xl z-20'>Expenses</h1>
                                    ...
                                    <ModalBody className="px-4 lg:px-8">
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-2  gap-3">
                                            ...
                                            <div className="flex flex-col col-span-2 lg:col-span-1 gap-1">
                                                ...
                                                <label htmlFor="LedgerName" className="block text-sm font-medium text-gray-700">
                                                    Ledger Name
                                                </label>
                                                <div className="mt-1 relative">
                                                    ...
                                                    />
                                                    {errors.selectedLedgerName && <span className="text-red-500 text-sm">{errors.selectedLedgerName}</span>}
                                                        ...
                                                        <ul ref={dropdownRef} className="mt-1  capitalize  absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-md overflow-y-auto max-h-60">
                                                            {LedgerNamedata.length === 0 ? (
                                                                ...
                                                                LedgerNamedata
                                                                    ...
                                                                    .map((item) => (
                                                                        <li key={item.Ledger.id} className="py-2 px-3 capitalize cursor-pointer hover:bg-gray-100" onClick={() => handleSelectLedgerName(item)}>
                                                                            {item.Ledger.name}
                                                                        </li>
                                                                    ))
                                                            )}
                ...
                <div className=" mt-28 mx-5 grid grid-cols-1 lg:mt-20 lg:grid-cols-2 gap-3 lg:gap-10">
                    {submittedData.map((data, index) => (
                        ...
                        data?.selectedLedgerName && (
                            <div key={index} className=" flex flex-col mt-5 lg:mt-10 justify-between lg:max-w-3xl max-w-sm lg:p-6 p-2 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">

                                <h5 className="lg:mb-1 mb-1 text-lg lg:text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{data.selectedLedgerName}</h5>
                                <div className="lg:my-2 my-1 grid grid-cols-2 lg:grid-cols-2 lg:gap-2 gap-1 lg:text-lg text-xs">
                                    {data.amount && <p className="text-gray-700 font-semibold">Amount: <span className="font-bold">{data.amount}</span> </p>}
                                </div>
                                <div className="lg:my-1 my-1 grid grid-cols-1 lg:grid-cols-1 lg:gap-2 gap-1 lg:text-lg text-xs">

                                    {data.coupenNo && <p className="text-gray-700 font-semibold">Narration: <span className="font-normal">{data.coupenNo}</span> </p>}
                                </div>
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <div className="mt-1 relative">
                                                        <input
                                                            ...
                                                            className="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                                        />
                                                        {errorss.selectedLedgerName && <span className="text-red-500 text-sm">{errorss.selectedLedgerName}</span>}
                                                        ...
                                                        {showDropdown && (
                                                            <ul ref={dropdownRef} className="mt-1 absolute z-10 w-full bg-white border border-gray-300 rounded-md shadow-md overflow-y-auto max-h-60">
                                                                {LedgerNamedata.length === 0 ? (
                                                                    ...
                                                                    LedgerNamedata
                                                                        .filter(item => item.Ledger.name.toLowerCase().includes(editData.selectedLedgerName.toLowerCase()))
                                                                        .map((item) => (
                                                                            <li key={item.Ledger.id} className="py-2 px-3 capitalize cursor-pointer hover:bg-gray-100" onClick={() => handleSelectLedgerName(item)}>
                                                                                {item.Ledger.name}
                                                                            </li>
